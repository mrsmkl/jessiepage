...
\subsection{Dragging points or text and source readback}
...
A standalone text widget with two literal coordinates works the same way:
...
Dragging either widget updates only its first two arguments. The displayed text may be static or dynamic. A computed position such as \code{text(map () -> X(A), ...)} remains controlled by its expression and is not rewritten.

\begin{tipbox}[Stable constructions]
Use ordinary points and literal-position text for inputs that students should drag. Use midpoints, intersections, gliders, map-defined points, and computed text positions for derived objects. This keeps the source readable and preserves the construction logic.
\end{tipbox}
